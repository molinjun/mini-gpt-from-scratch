...
Slide content:
- Slide number for internal tracking only: 11
- Filename: 11-slide-tokenizer-idea.png
- Type: Tokenizer
- Layout: linear-progression
- Narrative goal: Define tokenizer as the boundary from text to ids.
- Headline text to render: "Text must become token ids"
- Supporting text to render: "The model reads numbers, not letters on a screen."
- Small labels to render only where useful: text, tokens, ids, decode
- Visual description: Three-stage tokenizer pipeline: text string, token boxes, integer id boxes, then reverse decode arrow.
- Key points to show visually: Split text into tokens | Map tokens to ids | Decode ids back to text
