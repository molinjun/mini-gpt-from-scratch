...
Slide content:
- Slide number for internal tracking only: 06
- Filename: 06-slide-mlp-stack.png
- Type: Neural Network Basics
- Layout: hierarchical-layers
- Narrative goal: Show how many neurons become a small network.
- Headline text to render: "An MLP is layers of neurons"
- Supporting text to render: "Linear -> ReLU -> Linear computes new features."
- Small labels to render only where useful: input, Linear, ReLU, Linear, output
- Visual description: Layered MLP blueprint: input vector, first linear layer, ReLU row, second linear layer, output vector.
- Key points to show visually: Layer 1 creates hidden features | ReLU keeps useful positive signals | Layer 2 maps back to model size
