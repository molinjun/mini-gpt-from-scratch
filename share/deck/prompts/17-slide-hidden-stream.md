...
Slide content:
- Slide number for internal tracking only: 17
- Filename: 17-slide-hidden-stream.png
- Type: Embedding
- Layout: linear-progression
- Narrative goal: Introduce the vector stream that moves through the model.
- Headline text to render: "Each position carries a vector"
- Supporting text to render: "The Transformer edits these vectors layer by layer."
- Small labels to render only where useful: B, T, D, hidden stream
- Visual description: Tensor stream diagram: a row of token positions, each with a vertical vector bar, flowing into Transformer blocks.
- Key points to show visually: Shape: B, T, D | One vector per token position | Blocks update the stream
