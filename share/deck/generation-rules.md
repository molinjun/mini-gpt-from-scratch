...
- Avoid generic generated words such as `Machine` unless a slide explicitly needs that word.
...
- For `Dennis`, prefer lowercase character examples when showing IDs:
  - `d e n n`
  - `[4, 5, 14, 14]`
- Do not render large production-model token IDs such as `50256` or `100000` for Mini-GPT examples.
...
- QKV examples should use `I love CIS`, not unfamiliar generated words.
- Recap should use `Recap: Dennis Through Mini-GPT`, not `Recap: Dennis Through the Machine`.
